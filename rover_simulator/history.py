...
from rover_simulator.utils.draw import (
    ...
    draw_error_ellipses,
    draw_rover,
    draw_history_pose_with_error_ellipse,
)
...
class SimpleHistory(History):
    ...
    def draw(
        ...
        draw_rover(
            ax, self.real_poses[-1], self.rover_r, self.rover_color
        )  # Last rover position and angle
        draw_rover(
            ax, self.estimated_poses[-1], self.rover_r, self.rover_color
        )  # Last rover position and angle
        ...
        draw_pose(
            ax, self.waypoints[-1], self.waypoint_color
        ) if draw_waypoints_flag and self.waypoints is not None else None
    ...
    def animate_one_step(
        ...
        elems.append(ax.text(xlim[0] * 0.01, ylim[1] * 1.02, time_str, fontsize=10))
        ...
        draw_history_waypoints(
            ax, elems, self.waypoints, start_step + i
        ) if draw_waypoints_flag and len(self.waypoints) != 0 else None
        draw_history_pose(
            ax,
            elems,
            self.estimated_poses,
        ...
        draw_history_pose(
            ax, elems, self.real_poses, self.rover_r, self.rover_color, i, start_step
        )
...
class HistoryWithKalmanFilter(SimpleHistory):
    ...
    def draw(
        ...
        ) if draw_sensing_results_flag else None
        draw_rover(
            ax, self.real_poses[-1], self.rover_r, self.rover_color
        )  # Last rover position and angle
        draw_rover(
        ...
        draw_poses(ax, self.estimated_poses, self.rover_color, linestyle=":")
        draw_error_ellipses(
            ax, self.estimated_poses, self.estimated_covs, "blue", plot_step_uncertainty
        ) if draw_error_ellipse_flag else None
        draw_pose(
            ax, self.waypoints[-1], self.waypoint_color
        ) if draw_waypoints_flag and self.waypoints is not None else None
    ...
    def animate_one_step(
        ...
        start_step: int,
        draw_waypoints_flag: bool,
        draw_error_ellipse_flag: bool,
        draw_sensing_results_flag: bool,
        draw_sensing_points_flag: bool,
        draw_sensing_area_flag: bool,
        pbar,
    ):
        while elems:
            elems.pop().remove()
        ...
        if draw_sensing_results_flag:
            ax.plot(
                real_pose[0], real_pose[1], marker="o", c="red", ms=5
            ) if draw_sensing_points_flag is True else None
            self.sensor.draw(ax, elems, self.sensing_results[start_step + i], real_pose)
        draw_history_waypoints(
            ax, elems, self.waypoints, start_step + i
        ) if draw_waypoints_flag and len(self.waypoints) != 0 else None
