...
from rover_simulator.core import Rover, Obstacle, CircularObstacle, RectangularObstacle
from rover_simulator.utils.draw import (
    set_fig_params,
    draw_obstacles,
    draw_poses,
    draw_waypoints,
    draw_sensing_results,
    draw_start,
    draw_goal,
    draw_rover,
    draw_history_pose,
    draw_history_waypoints,
)
...
class World:
    ...
    def simulate(
        self, steps: int = 100, tqdm_leave: bool = True, tqdm_desc="Simulation"
    ):
        ...
        while dist_to_goal >= goal_range:
            ...
            if len(rover_poses) > stuck_step:
                ...
                for i in range(stuck_step):
                    traversed_dist += np.linalg.norm(
                        rover_poses[i - stuck_step - 1][0:2]
                        - rover_poses[i - stuck_step][0:2]
                    )
            ...
            if f_line[0] == "Obstacle":
                if f_line[1] == "Circle":
                    self.append_obstacle(
                        CircularObstacle(
                            np.array([float(f_line[2]), float(f_line[3])]),
                            float(f_line[4]),
                        )
                    )
    ...
    def set_start_goal_ramdomly(
        self,
        x_range: list[float] = [0, 20],
        y_range: list[float] = [0, 20],
        min_dist: float = 0.0,
        max_dist: float = 20,
        expand_dist=0.0,
    ) -> list[np.ndarray]:
        ...
        while distance < min_dist or distance > max_dist or is_collision:
            ...
            if not (
                x_range[0] <= goal_pos[0] <= x_range[1]
                and y_range[0] <= goal_pos[1] <= y_range[1]
            ):
                continue
            ...
            if obstacle_kdTree:
                ...
                if (
                    dist_start < self.obstacles[idx_start].r + expand_dist + 1.0
                    or dist_goal < self.obstacles[idx_goal].r + expand_dist + 1.0
                ):
                    is_collision = True
                else:
                    is_collision = False
    ...
    def reset(
        ...
        reset_rovers: bool = True,
        reset_obstacles: bool = True,
    ):
    ...
    def draw(
        ...
        for rover in self.rovers:
            if rover.history:
                ...
                draw_poses(
                    self.ax, rover.history.estimated_poses, rover.color, linestyle=":"
                )
                if draw_sensing_results_flag:
                    draw_sensing_results(
                        self.ax,
                        rover.history.real_poses,
                        rover.sensor.range,
                        rover.sensor.fov,
                        rover.sensing_results,
                        draw_sensing_points_flag,
                        draw_sensing_area_flag,
                    )
            draw_rover(self.ax, rover.real_pose, rover.r, rover.color)
            draw_waypoints(
                self.ax, rover.waypoints, rover.waypoint_color
            ) if draw_waypoints_flag and rover.waypoints is not None else None
    ...
    def animate_one_step(
        ...
        elems: list,
        start_step: int,
        draw_waypoints_flag: bool,
        draw_sensing_points_flag: bool,
        draw_sensing_results_flag: bool,
        pbar,
    ):
        while elems:
            elems.pop().remove()
        ...
        for rover in self.rovers:
            if rover.history:
                ...
                draw_history_waypoints(
                    ax, elems, rover.history.waypoints, start_step + i
                ) if draw_waypoints_flag and len(rover.history.waypoints) != 0 else None
