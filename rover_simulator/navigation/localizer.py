...
class NoisyLocalizer(ImaginalLocalizer):
    def __init__(
        self,
        noise: np.ndarray = np.array([0.2, 0.05]),
        bias: np.ndarray = np.array([1.1, 1.01]),
    ) -> None:
        super().__init__()
        if not noise.shape == (2,):
            raise ValueError(
                "array 'noise' is not of the right shape (2,), given array's shape is {}".format(
                    noise.shape
                )
            )
        if not bias.shape == (2,):
...
class KalmanFilter:
    def __init__(
        ...
        init_pose,
        init_cov=None,
        motion_noise_stds={"nn": 0.1, "no": 0.0001, "on": 0.013, "oo": 0.02},
    ):
        if init_cov is None:
    ...
    def estimate_pose(self, previous_pose: np.ndarray, v: float, w: float, dt: float):
        ...
        cov = covariance_transition(
            previous_pose, self.belief.cov, self.motion_noise_stds, v, w, dt
        )
