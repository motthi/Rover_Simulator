...
from rover_simulator.utils.utils import (
    is_angle_in_range,
    set_angle_into_range,
    is_in_list,
)
...
class GridMapper(Mapper):
    def __init__(
        ...
        for obstacle in known_obstacles:
            if obstacle.type == "circular":
                ...
                self.update_rectangle(
                    obstacle.xy, obstacle.w, obstacle.h, obstacle.angle, 1.0
                )
        ...
        if sensor:
            ...
            for i in range(
                np.ceil(-sensing_range).astype(np.int32),
                np.floor(sensing_range).astype(np.int32) + 1,
            ):
                for j in range(
                    np.ceil(-sensing_range).astype(np.int32),
                    np.floor(sensing_range).astype(np.int32) + 1,
    ...
    def update(self, rover_estimated_pose: np.ndarray, sensing_results) -> None:
        ...
        if self.sensor.type == "stereo_camera":
            ...
            ang_range_min = set_angle_into_range(
                rover_estimated_pose[2] - self.sensor.fov / 2
            )
            ang_range_max = set_angle_into_range(
                rover_estimated_pose[2] + self.sensor.fov / 2
            )
            ...
            for pt in sensing_results:
                ...
                self.update_circle(
                    pt + rover_estimated_pose[:2], self.expand_dist, 0.95
                )
            ...
            for [r, th] in sensing_results:
                ...
                pt = rover_estimated_pose[:2] + np.array(
    ...
    def update_circle(self, pos: np.ndarray, r: float, occupancy: float):
        ...
        for lattice_x in lattice_range:
            for lattice_y in lattice_range:
                dist2 = (np.fabs(lattice_x) - grid_width / 2) ** 2 + (
                    np.fabs(lattice_y) - grid_width / 2
                ) ** 2
                if dist2 > r2:
                    continue  # 円の外なら無視
    ...
    def update_rectangle(self, xy, width, height, angle, occupancy):
        ...
        updated_grids = []
        range_x = np.arange(
            -self.expand_dist, self.expand_dist + width + 1e-5, self.grid_width * 0.5
        )
        range_y = np.arange(
            -self.expand_dist, self.expand_dist + height + 1e-5, self.grid_width * 0.5
        )
...
class TableMapper(Mapper):
    ...
    def update(
        self, rover_estimated_pose: np.ndarray, sensed_obstacles: list[dict]
    ) -> None:
        # list up obstacles in sensing range
        obstacle_in_sense_idxes = []
        if self.obstacle_kdTree is not None:
            idxes = self.obstacle_kdTree.query_ball_point(
                rover_estimated_pose[0:2], r=self.sensor.range
            )
            for idx in idxes:
        ...
        if self.obstacle_kdTree is not None:
            idxes = self.obstacle_kdTree.query_ball_point(
                rover_estimated_pose[0:2], r=self.retain_range
            )
            obstacle_outofrange_idxes = [
                i for i in range(len(self.obstacles_table)) if i not in idxes
            ]

        # Delete Obstacles
        for idx in sorted(
            set(obstacle_in_sense_idxes + obstacle_outofrange_idxes), reverse=True
        ):
        ...
        for sensed_obstacle in sensed_obstacles:
            ...
            obstacle_pos = rover_estimated_pose[0:2] + np.array(
        ...
        if not len(self.obstacles_table) == 0:
            obstacle_positions = [
                obstacle.pos[0:2] for obstacle in self.obstacles_table
            ]
