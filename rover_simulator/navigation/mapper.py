...
class GridMapper(Mapper):
    ...
    def update(self, rover_estimated_pose: np.ndarray, rover_r: float, sensed_obstacles: List[Dict]):
        ...
        for i in sorted(deleted_obstacles, reverse=True):
            ...
            _ = self.update_circle(obstacle_idx, radius + rover_r, 0.01)
            self.obstacles_table.pop(i)

        # List up all obstacles
        updated_grids = []
        for idx, radius in new_obstacles:
            updated_grids += self.update_circle(idx, radius + rover_r, 0.99)
