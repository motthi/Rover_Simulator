...
from rover_simulator.utils.cmotion.cmotion import (
    state_transition,
...
class RRT(PathPlanner):
    ...
    def __init__(
        ...
        self.start_node = (
            self.Node(start_pos[0], start_pos[1]) if start_pos is not None else None
        )
        self.goal_node = (
            self.Node(goal_pos[0], goal_pos[1]) if goal_pos is not None else None
        )
        ...
        for _ in range(max_iter):
            ...
            if (
                ...
                final_node = self.steer(
                    self.node_list[-1], self.goal_node, self.expand_dis
                )
    ...
    def steer(
        ...
        new_node.cost = from_node.cost + self.cost(
            self.to_pose(from_node), self.to_pose(new_node)
        )
        return new_node
    ...
    def check_collision(self, node: Node, obstacle_list: list[Obstacle]):
        ...
        for obs in obstacle_list:
            for x1, y1, x2, y2 in zip(
                node.path_x[:-1], node.path_y[:-1], node.path_x[1:], node.path_y[1:]
            ):
                if obs.check_collision_line(
                    np.array([x1, y1]), np.array([x2, y2]), self.expand_dist
                ):
    ...
    def calc_distance_and_angle(self, from_node, to_node):
        return self.distance(from_node, to_node), self.node_heading_angle(
            from_node, to_node
        )
    ...
    def animate_one_step(
        self, i: int, ax: Axes, elems: list, xlim: list, ylim: list, pbar
    ):
        while elems:
            elems.pop().remove()
...
class RRTstar(RRT):
    ...
    def calculate_path(self, max_iter=500, log_history=False, *kargs):
        ...
        for _ in range(max_iter):
            ...
            new_node.cost = nearest_node.cost + self.cost(
                self.to_pose(new_node), self.to_pose(nearest_node)
            )
            ...
            if log_history:
                ...
                if (
                    ...
                    and self.calc_dist_to_goal(new_node.x, new_node.y) < 1e-5
                ):
...
class InformedRRTstar(RRTstar):
    def __init__(self, *args, **kwargs):
        ...
        self.e_theta = np.arctan2(
            self.goal_pos[1] - self.start_pos[1], self.goal_pos[0] - self.start_pos[0]
        )
    ...
    def sample_new_node(self) -> RRT.Node:
        if self.reached_goal:
            ...
            x = np.sqrt(random.random()) * np.array(
                [np.cos(theta) * self.a, np.sin(theta) * self.b]
            )
            xp = (
                np.dot(rotation_matrix(self.e_theta), x)
                + (self.start_pos[:2] + self.goal_pos[:2]) / 2
            )
...
class ChanceConstrainedRRT(RRT):
    class Node(RRT.Node):
        def __init__(
            ...
            y: float,
            head: float = 0.0,
            cov: np.ndarray = np.diag([1e-10, 1e-10, 1e-10]),
        ) -> None:
            self.x = x
            self.y = y
            self.head = head
    ...
    def __init__(
        ...
        start_pos: np.ndarray = None,
        goal_pos: np.ndarray = None,
        start_cov: np.ndarray = None,
        start_head: float = None,
        ...
        cost_func=None,
        steer_func=None,
        goal_sample_rate: float = 0.01,
        goal_region=2.0,
        num_nearest_node: int = 6,
        p_safe: float = 0.99,
        k: float = 1.0,
    ) -> None:
        super().__init__(
        ...
        if start_pos is not None and goal_pos is not None and start_cov is not None:
            if start_head is None:
                start_head = np.arctan2(
                    goal_pos[1] - start_pos[1], goal_pos[0] - start_pos[0]
                )
                start_head = set_angle_into_range(start_head)
            self.start_node = self.Node(
                start_pos[0], start_pos[1], start_head, start_cov
            )
            self.goal_node = self.Node(goal_pos[0], goal_pos[1], None, None)
        self.num_nearest_node = num_nearest_node
        self.p_safe = p_safe
        self.motion_noise_stds = np.array(
            [
                motion_noise_stds["nn"],
                motion_noise_stds["no"],
                motion_noise_stds["on"],
                motion_noise_stds["oo"],
            ]
        )
        self.sampled_pts = []
        ...
        self.goal_pos = goal_pos
        self.e_theta = np.arctan2(
            start_pos[1] - goal_pos[1], start_pos[0] - goal_pos[0]
        )
        self.cnt = 0
        self.dt = 0.5
    ...
    def calculate_path(self, max_iter: int = 200, log_history=False, *kargs) -> list:
        ...
        self.nodes_history.append(
            copy.deepcopy(self.node_list)
        ) if log_history is True else None
        for _ in range(max_iter):
            self.expand_tree()
            self.nodes_history.append(
                copy.copy(self.node_list)
            ) if log_history is True else None
    ...
    def generate_final_course(self):
        ...
        idxes = nodes_kdtree.query_ball_point(
            np.array([self.goal_node.x, self.goal_node.y]), r=self.goal_region
        )
        min_cost = float("inf")
        min_idx = None
        for idx in idxes:
    ...
    def expand_tree(self) -> None:
        ...
        near_nodes = self.get_m_nearest_nodes(
            self.node_list, trg_node, self.num_nearest_node
        )
        for near_node in near_nodes:
            ...
            if len(new_nodes) == 1:
                if (
                    math.hypot(
                        new_nodes[0].x - near_node.x, new_nodes[0].y - near_node.y
                    )
                    < 0.01
                ):
                    continue
            for new_node in new_nodes:
                new_node.cost_fs = new_node.parent.cost_fs + self.cost(
                    self.to_pose(new_node.parent), self.to_pose(new_node)
                )
                self.node_list.append(new_node)
                nodes_to_goal, reached_goal = self.connect_to_target(
                ...
                if reached_goal is True:
                    ...
                    while not node.parent is None:
                        ...
                        lb_cost = self.cost(
                            self.to_pose(self.goal_node), self.to_pose(node)
                        )
                        if ub_cost < lb_cost:
    ...
    def connect_to_target(
        ...
        while dist < expand_dis:
            ...
            is_far_last_node = (
                math.hypot(prev_node.x - node.x, prev_node.y - node.y)
                > self.path_resolution
            )
            if is_near_trg or is_far_last_node:
                node.cost_lb = self.cost(
                    self.to_pose(node), self.to_pose(self.goal_node)
                )
                self.connect_nodes(prev_node, node)
                new_nodes.append(node)
                prev_node = node
                node = self.Node(
                    prev_node.x, prev_node.y, prev_node.head, prev_node.cov
                )
                node.cost_fs = prev_node.cost_fs
                if is_near_trg:
                    break
        if flag_safe is False:
            if not node.x == prev_node.x and node.y == prev_node.y:
                ...
                new_nodes.append(node)
        return new_nodes, (
            dist_to_trg < self.expand_dis and len(new_nodes) > 0
        ) and flag_safe

    def update_node(
        self, node: Node, prev_pose: np.ndarray, new_pose: np.ndarray, cov: np.ndarray
    ) -> Node:
        node.x = new_pose[0]
        node.y = new_pose[1]
    ...
    def get_m_nearest_nodes(
        self, node_list: list[Node], rnd_node: Node, m: int
    ) -> list[Node]:
        dist_costs = [
            self.dist_nodes(self.to_pose(node), self.to_pose(rnd_node))
            for node in node_list
        ]  # rnd_nodeから各nodeへの距離
        fs_costs = [
            node.cost_fs for node in node_list
        ]  # スタート地点から各nodeまでの距離
    ...
    def select_inputs(self, cur_pose: np.ndarray, trg_pose: np.ndarray) -> list:
        theta = (
            math.atan2(trg_pose[1] - cur_pose[1], trg_pose[0] - cur_pose[0])
            - cur_pose[2]
        )
    ...
    def steer(
        self, prev_pose: np.ndarray, prev_cov: np.ndarray, nu: float, omega: float
    ) -> Node:
        if abs(omega) < 1e-5:
            omega = 1e-5  # 値が0になるとゼロ割りになって計算ができないのでわずかに値を持たせる
        new_pose = state_transition(prev_pose, nu, omega, self.dt)
        new_cov = covariance_transition(
            prev_pose, prev_cov, self.motion_noise_stds, nu, omega, self.dt
        )
...
class ChanceConstrainedRRTstar(ChanceConstrainedRRT):
    def __init__(
        ...
        self.mu = mu
        self.mu_xfree = (explore_region[0][1] - explore_region[0][0]) * (
            explore_region[1][1] - explore_region[1][0]
        )
    ...
    def expand_tree(self) -> None:
        ...
        for n in near_m_nodes:
            nodes_min, succeed = self.connect_to_target(
                n, trg_node, connect_to_end=True
            )
            if succeed:
                break
        if succeed:
            ...
            for n in near_nodes:
                nodes, succeed = self.connect_to_target(
                    n, trg_node, connect_to_end=True
                )
            ...
            # Rewiring
            near_nodes = self.get_near_nodes_except_ancesters(
                self.node_list, trg_node, nodes_min[-1]
            )
            ...
            for near_node in near_nodes:
                ...
                nodes, succeed = self.connect_to_target(
                    near_node, nodes_min[-1], connect_to_end=True
                )
                if succeed:
                    if nodes[-1].cost_fs < cost_min:
                        if near_node.parent:
    ...
    def calc_new_cost(
        self, from_node: ChanceConstrainedRRT.Node, to_node: ChanceConstrainedRRT.Node
    ):
        d = self.cost(self.to_pose(from_node), self.to_pose(to_node))
    ...
    def get_near_nodes_except_nearest(
        self, node_list: list, trg_node: ChanceConstrainedRRT.Node
    ) -> list[ChanceConstrainedRRT.Node]:
        # max_num = 10
    ...
    def get_near_nodes_except_ancesters(
        self,
        node_list: list,
        trg_node: ChanceConstrainedRRT.Node,
        n_min: ChanceConstrainedRRT.Node,
    ) -> list[ChanceConstrainedRRT.Node]:
        # max_num = 10
        near_idxes = []
