...
class SkeltonPlanner(PathPlanner):
    ...
    def skeletonize(self, gridmap):
        ...
        v_map = cv2.resize(
            v_map, (v_map.shape[0] * self.rescale, v_map.shape[1] * self.rescale)
        )
        # v_map = v_map.T
        ...
        skeleton = cv2.ximgproc.thinning(
            bin_route, thinningType=cv2.ximgproc.THINNING_GUOHALL
        )
        return skeleton
    ...
    def calculate_path(self):
        path = nx.astar_path(
            self.graph, self.start_node, self.goal_node, weight="weight"
        )
        ...
        self.path = np.concatenate(
            [np.array([self.start]), self.path, np.array([self.goal])], axis=0
        )
        return self.path
    ...
    def draw(
        ...
        extent = (
            ...
            -mapper.grid_width / 2,
            mapper.grid_width * mapper.grid_num[1],  # - mapper.grid_width / 2
        )
