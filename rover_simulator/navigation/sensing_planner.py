...
class SimpleSensingPlanner(SensingPlanner):
    ...
    def decide(self, rover_pose: np.ndarray) -> None:
        ...
        d_theta = set_angle_into_range(
            np.arctan2(
                rover_pose[1] - self.previous_sensing_pose[1],
                rover_pose[0] - self.previous_sensing_pose[0],
            )
            - self.previous_sensing_pose[2]
        )
        if dist > 1e-5 and not is_angle_in_range(
            d_theta, -self.sense_interval_angle, self.sense_interval_angle
        ):
            # dist > 1e-5 => Not Moved
            self.previous_sensing_pose = rover_pose
            return True
