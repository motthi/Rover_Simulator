...
def draw_pose(
    ax: Axes, pose: np.ndarray, color: str = "black", alpha: float = 1.0
) -> None:
    ax.plot(
        [e[0] for e in pose],
        [e[1] for e in pose],
...
def draw_grid_map_contour(
    ax: Axes, grid_map: np.ndarray, grid_num: list, grid_width: float, levels
):
...
def draw_sensing_results(
    ...
    for i, sensing_result in enumerate(sensing_results):
        if sensing_result is None:
            continue
        ax.plot(
            poses[i][0], poses[i][1], marker="o", c="red", ms=5
        ) if draw_sensing_points_flag else None
...
def draw_history_pose(
    ...
    xn, yn = poses[start_step + step][0:2] + rover_r * np.array(
        [np.cos(poses[start_step + step][2]), np.sin(poses[start_step + step][2])]
    )
    elems += ax.plot(
        [poses[start_step + step][0], xn],
        [poses[start_step + step][1], yn],
        color=rover_color,
    )
    elems += ax.plot(
    ...
    c = patches.Circle(
        xy=(poses[start_step + step][0], poses[start_step + step][1]),
        radius=rover_r,
        fill=False,
        color=rover_color,
    )
...
def draw_history_pose_with_error_ellipse(
    ax: Axes,
    elems: list,
    poses: list,
    covs: list,
...
def draw_history_waypoints(
    ax: Axes, elems: list, waypoints_list: list, step: int
) -> None:
